...
PLoop(function(_ENV)
    ...
    __Sealed__() struct "PositiveNumber"  	{ __base = Number, function(val, onlyvalid) return val <= 0 and (onlyvalid or "the %s must be a positive number") or nil end }

    --- Represents the negative number
    __Sealed__() struct "NegativeNumber"  	{ __base = Number, function(val, onlyvalid) return val >= 0 and (onlyvalid or "the %s must be a negative number") or nil end }

    --- Represents negative integer value
    __Sealed__() struct "NegativeInteger"  	{ __base = Integer, function(val, onlyvalid) return val >= 0 and (onlyvalid or "the %s must be a negative integer") or nil end }
end)