...
**如果你对PLoop的代码编写方式没有什么问题，可以跳过本章，或者直接查看本章最后的[System.Module](#systemmodule)部分**
...
* [System.Environment](#systemenvironment)
	...
	* [SetParent](#setparent)
* [System.Module 模组-树状的代码管理系统](#systemmodule_模组-树状的代码管理系统)
	* [版本控制](#版本控制)
	* [其他可用资源](#其他可用资源)
	* [创建新模组类型](#创建新模组类型)
...
3. 检查公共命名空间（目前`System`和`System.Collections`），尝试匹配名字或下属类型的名字，如果匹配成功，返回对应类型。

4. 每个**PLoop**提供的环境都可以指定基础环境，通常这个环境都是`_G`，上述查找失败后，会尝试从基础环境中读取该名称的全局变量，如果读到，返回值。
