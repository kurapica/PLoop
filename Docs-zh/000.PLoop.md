...
错误验证更常用在函数（普通函数，对象方法等）上，我们经常需要对函数的调用参数进行校验，已准确给调用者反馈信息，但参数校验是比较麻烦的，特别如果参数结构比较复杂，校验代码很占用大量的业务逻辑，并且一旦正式发布，不需要做错误验证时，我们也需要屏蔽这部分验证逻辑。
...
直接使用协程池并不方便，需要访问协程池，请求协程再使用，通过特性我们可以将这整个操作覆盖掉：
