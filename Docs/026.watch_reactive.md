...
So if we need know deep value changes, we must declare the data structure, so we can define a member struct type, then use as the type template to the `reactive`, since the data struct is clear, the system will track the deep data changes.
...
Since we declare the point is also a struct, the `data.point` also is a `Reactive` object, we can use it standalone.
