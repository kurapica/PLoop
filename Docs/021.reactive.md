...
If we create an observable to check an observer's subscription, we must use the `IsUnsubscribed` property to check if the observer has cancelled the subscription, __please don't use the event, it may cause the memeory leak that the observable may have a reference in the event handler which make it un-collectable__.
