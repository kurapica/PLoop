...
We create a reactive table `data` with one observable field `name` and a reactive table `info`, the `info` also has an observable field `age`.

Then we create a watch function, the function will be processed with a private environment immediality, the environment will track the observable or reactive data the function accessed and do the subscriptions automatically.

So when the `name` or `info.age` changed, the watch function will be processed again.
